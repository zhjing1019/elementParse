...
import ElTableBody from "./table-body";
import ElTableHeader from "./table-header";
import ElTableFooter from "./table-footer";
...
export default {
  ...
  computed: {
    headerHeight() {
      let height = 0;
      if (this.store.states && this.store.states.columns) {
        height = this.layout.headerHeight * this.store.states.columns.length;
      }
      return height;
    },
    tableSize() {
  ...
  watch: {
    ...
    expandRowKeys: {
      ...
      handler(newVal) {
        if (newVal) {
          console.log(this.store.setExpandRowKeys(newVal));
